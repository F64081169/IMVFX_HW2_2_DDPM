...
class UNet(nn.Module):
    def __init__(self, n_steps=1000, time_embedding_dim=256):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.Conv2d(3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(8*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        #input: 64*64 output: 32*32
        self.down1 = nn.Conv2d(8*3, 8*3, 4, 2, 1)

        self.time_step_encoder2 = nn.Sequential(
            nn.Linear(time_embedding_dim, 8*3),
            nn.SiLU(),
            nn.Linear(8*3, 8*3)
        )
        #input: 32*32 output: 32*32
        self.block2 = nn.Sequential(
            nn.LayerNorm((8*3, 32, 32)),
            nn.Conv2d(8*3, 16*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(16*3, 16*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        #input: 32*32 output: 16*16
        self.down2 = nn.Conv2d(16*3, 16*3, 4, 2, 1)

        self.time_step_encoder3 = nn.Sequential(
            nn.Linear(time_embedding_dim, 16*3),
            nn.SiLU(),
            nn.Linear(16*3, 16*3)
        )
        #input: 16*16 output: 16*16
        self.block3 = nn.Sequential(
            nn.LayerNorm((16*3, 16, 16)),
            nn.Conv2d(16*3, 32*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(32*3, 32*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        ...
        self.down3 = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Conv2d(32*3, 32*3, 3, 2, 1)
        )

        # The bottleneck
        self.time_step_encoder_mid = nn.Sequential(
            nn.Linear(time_embedding_dim, 32*3),
            nn.SiLU(),
            nn.Linear(32*3, 32*3)
        )
        #input: 8*8 output: 8*8
        self.block_mid = nn.Sequential(
            nn.LayerNorm((32*3, 8, 8)),
            nn.Conv2d(32*3, 32*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(32*3, 32*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        ...
        self.up1 = nn.Sequential(
            ...
            #nn.LeakyReLU(0.2),
            #nn.ConvTranspose2d(32, 32, 2, 1)
            nn.ConvTranspose2d(32*3, 32*3, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.2),
            nn.ConvTranspose2d(32*3, 32*3, kernel_size=3, stride=1, padding=1),
        )

        self.time_step_encoder4 = nn.Sequential(
            nn.Linear(time_embedding_dim, 64*3),
            nn.SiLU(),
            nn.Linear(64*3, 64*3)
        )
        #input: 16*16 output: 16*16
        self.block4 = nn.Sequential(
            nn.LayerNorm((64*3, 16, 16)),
            nn.Conv2d(64*3, 16*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(16*3, 16*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        #input: 16*16 output: 32*32
        self.up2 = nn.ConvTranspose2d(16*3, 16*3, 4, 2, 1)
        ...
        #input: 32*32 output: 32*32
        self.block5 = nn.Sequential(
            nn.LayerNorm((32*3, 32, 32)),
            nn.Conv2d(32*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(8*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        #input: 32*32 output: 64*64
        self.up3 = nn.ConvTranspose2d(8*3, 8*3, 4, 2, 1)
        ...
        #input: 64*64 output: 64*64
        self.block6 = nn.Sequential(
            nn.LayerNorm((16*3, 64, 64)),
            nn.Conv2d(16*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(8*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
            nn.LayerNorm((8*3, 64, 64)),
            nn.Conv2d(8*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(8*3, 8*3, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
        #input: 64*64 output: 64*64
        self.final_layer = nn.Conv2d(8*3, 3, 3, 1, 1)
...
generated_images = generate_new_images(ddpm_anime)
output_path = '/kaggle/working/output'
os.makedirs(output_path, exist_ok=True)
for i, img in enumerate(generated_images):
    img_path = os.path.join(output_path, f"generated_image_{i + 1}.png")
    vutils.save_image(img, img_path)
    
real_images = '/kaggle/working/anime_face_dataset/data'

# FID score
fid_value = fid_score.calculate_fid_given_paths([real_images, output_path], batch_size=64, dims=64, device='cuda')
print(f"FID Score: {fid_value}")
# fid score
# the code reference from: https://blog.csdn.net/qq_40608730/article/details/110546612
#!python -m pytorch_fid f"{workspace_dir}/anime_face_dataset"  f"{workspace_dir}/generated_images"
